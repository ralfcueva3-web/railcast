...
async def lifespan(app:FastAPI):
    ...
    try: await app.state.db.connect()
    except Exception: app.state.db=None
    predictor.load()
    yield
    if app.state.db: await app.state.db.close()
    await app.state.redis.close()

app=FastAPI(title="RailCast API",version="1.0.0",lifespan=lifespan)
...
async def health():
    ...
    return HealthResponse(status="ok" if db_ok and redis_ok and predictor.ready else "degraded",
                          database="up" if db_ok else "down",redis="up" if redis_ok else "down",
...
async def token(username:str,password:str):
    ...
    if username!=expected_user or password!=expected_password: raise HTTPException(401,"Invalid credentials")
    exp=datetime.now(timezone.utc)+timedelta(hours=12)
    return TokenResponse(access_token=jwt.encode({"sub":username,"exp":exp},os.getenv("JWT_SECRET","change-me"),algorithm="HS256"))

@app.get("/")
async def root():
    return {"service":"RailCast API","problem_statement":"26028","train":13028}
